...
use crate::fifo::{fold_core, Lot, NoopSink};
...
impl Checkpoint {
    ...
    pub fn build(table: &PackedTable, cutoff_day: i32) -> Self {
        ...
        for p in 0..pc.len() {
            ...
            fold_core(pc[p], ps[p], &mut carry, in_scope, &mut NoopSink, None);
            if !carry.is_empty() {
...
mod tests {
    ...
    fn range_query_via_checkpoint_matches_truth() {
        ...
        let truth = {
            ...
            fold_core(1, 1, &mut carry, &recs, &mut NoopSink, Some((150, 250)))
        };
        ...
        let got = fold_core(1, 1, &mut carry, replay, &mut NoopSink, Some((150, 250)));
